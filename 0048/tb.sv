`timescale 1ns / 1ps

module tb #(
     //---------------------------------------------------------
    // Input Vectors
    parameter DATA_WIDTH = 8,
    parameter signed INPUT1_1 = 10,
    parameter signed INPUT1_2 = 20,
    parameter signed INPUT1_3 = 15,
    parameter signed INPUT1_4 = 25,
    parameter signed INPUT1_5 = 30,
    parameter signed INPUT1_6 = -10
    );

    // --------------------------------------------------------

    localparam TB_SIM_TIMEOUT = 30_000_000; //ns

reg clk;
reg reset;
reg signed [DATA_WIDTH-1:0] data_in;
wire signed [DATA_WIDTH:0] data_out;

int ERR_COUNT = 0;

//DUT instantiation
   digital_differentiator #(
      .DATA_WIDTH(DATA_WIDTH)
   ) DUT (
      .clk(clk),
      .reset(reset),
      .data_in(data_in),
      .data_out(data_out)
   );

// Clock generation
initial begin
    clk = 0;
    forever #10 clk = ~clk;
end

// stimulus/test sequence
initial begin
    // Initialize inputs
    reset = 1;
    data_in = 0;
    @(posedge clk);
    @(posedge clk);
    #2;
    reset = 0;

    // Test sequence
    @(posedge clk);
    #2;
    data_in = INPUT1_1;
    $display("%0tns Applying data_in = %0d", $time, data_in);
    @(posedge clk);
    #2;
    data_in = INPUT1_2;
    $display("%0tns Applying data_in = %0d", $time, data_in);
    @(posedge clk);
    #2;
    data_in = INPUT1_3;
    $display("%0tns Applying data_in = %0d", $time, data_in);
    @(posedge clk);
    #2;
    data_in = INPUT1_4;
    $display("%0tns Applying data_in = %0d", $time, data_in);
    @(posedge clk);
    #2;
    data_in = INPUT1_5;
    $display("%0tns Applying data_in = %0d", $time, data_in);
    @(posedge clk);
    #2;
    data_in = INPUT1_6;
    $display("%0tns Applying data_in = %0d", $time, data_in);
    @(posedge clk);
    @(posedge clk);

    check_result; //must call this task to end the simulation
end

// golden solution
reg signed [DATA_WIDTH-1:0] prev_sample;
reg signed [DATA_WIDTH:0] expected_out;

always @(posedge clk, posedge reset) begin
    if (reset) begin
        prev_sample <= 0;
        expected_out <= 0;
    end else begin
        prev_sample <= data_in;
        expected_out <= data_in - prev_sample;
    end
end

always @(posedge clk) begin
    if (!reset) begin
        #1; // Small delay to check output after clock edge
        if(data_out !== expected_out) begin
            ERR_COUNT = ERR_COUNT + 1;
            $error("%0tns Data_out = %0d, expected = %0d", $time, data_out, expected_out);
        end else begin
            $display("%0tns Data_out = %0d,", $time, data_out);
        end
    end
end


//do not edit below
task check_result;
begin
   if(ERR_COUNT > 0) begin
      $display("Test failed with %0d errors.", ERR_COUNT);
   end else begin
      $display("Test PASS");
   end
   $finish;
end
endtask

string filename;

initial begin
   if ($value$plusargs("VCDFILE=%s",filename)) begin
      $dumpfile(filename);
      $dumpvars(0, DUT);
   end
end

initial begin
    #(TB_SIM_TIMEOUT)
    $display("Simulation TIMEOUT");
    $finish;
end

endmodule